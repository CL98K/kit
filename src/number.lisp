...
(defun bytes-to-integer (array &key (order :big) (signed nil))
  "字节数组转数字"
  (if (and (not (eq order :big)) (not (eq order :little))) (error "The 'order' can only be: big or :little"))
  
  (let* ((integer 0)
         (element (if signed 0 1))
         (bits-length (* (array-total-size array) 8))
         (bits (make-array bits-length :element-type 'bit :initial-element (if signed 1 0))))
    (declare (type bit element) (type fixnum bits-length) (type integer integer) (type simple-array bits))
    
    (loop for integer fixnum across (if (eq order :big) array (nreverse array))
          for i fixnum from 0 by 8
          do
          (loop for v fixnum downfrom 7 to 0
                for j fixnum from i
                do
                (if signed              
                    (if (not (logbitp v integer)) (setf (aref bits j) element))
                    (if (logbitp v integer) (setf (aref bits j) element)))))
